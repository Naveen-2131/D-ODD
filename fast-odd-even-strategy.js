...
class FastOddEvenStrategy { constructor(config = {}) { this.bot = null; this.isRunning = false;
...
async onTick(tick) {
    ...
    // --- MAIN STRATEGY (2× ODD → EVEN, 2× EVEN → ODD) ---
    if (this.currentMode === 'ODD_EVEN') {
        if (this.tickHistory.length >= 2) {
            const last1 = this.tickHistory[this.tickHistory.length - 1];
            const last2 = this.tickHistory[this.tickHistory.length - 2];

            // IF 2× ODD → TRADE EVEN
            if (last1 % 2 === 1 && last2 % 2 === 1) {
                console.log(`⚡ Signal: Two ODD digits (${last2}, ${last1}) → Trade EVEN`);
                await this.executeTrade('DIGITEVEN');
                return;
            }

            // IF 2× EVEN → TRADE ODD
            if (last1 % 2 === 0 && last2 % 2 === 0) {
                console.log(`⚡ Signal: Two EVEN digits (${last2}, ${last1}) → Trade ODD`);
...
module.exports = FastOddEvenStrategy;        this.isInCooldown = false;
        this.cooldownDuration = config.cooldownDuration || 120000;
        this.cooldownEndTime = null;
    }
